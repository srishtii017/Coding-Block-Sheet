...
class Main {
	static class BinaryTree {
		...
		private Info largestBSTinBT(Node root) {
			...
			Info currInfo = new Info(1, root.data, root.data, 0, true);  

         
        if (leftInfo.isBST && rightInfo.isBST && leftInfo.max < root.data && rightInfo.min > root.data) {  
            currInfo.size = leftInfo.size + rightInfo.size + 1;  
            currInfo.max = Math.max(root.data, rightInfo.max);  
            currInfo.min = Math.min(root.data, leftInfo.min);  
            currInfo.ans = currInfo.size; 
            currInfo.isBST = true;  
        } else {  
            currInfo.size = 1;  
            currInfo.max = root.data;  
            currInfo.min = root.data;  
